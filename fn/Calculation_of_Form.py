...
class loc_process:
    ...
    def mechanic_calculate(self, pix_thresh=5, scale=False):
        ...
        for seq, path in zip(self.locSeq, self.path):
            ...
            ##########################################################
            locSeq = assist_index.yield_nodeSeq(seq, sample_interval=20)
            ##########################################################
            update = []
            for coSeq in locSeq:
        ...
        if scale == False:
            ...
            update_list["interia"] = update_list["interia"]
